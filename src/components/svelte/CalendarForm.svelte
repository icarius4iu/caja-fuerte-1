<script lang="ts">
  ...
  function handleSubmit() {
    console.log("Nueva Promoción:", { title, category, startDate, endDate, benefit, imageUrl });
...
<div class="bg-secondary/50 rounded-xl border border-white/5 p-6 backdrop-blur-sm">
  <h2 class="text-2xl font-bold text-white font-display mb-6">Agregar Nueva Promoción</h2>

  <form on:submit|preventDefault={handleSubmit} class="space-y-6 max-w-2xl">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div class="col-span-1 md:col-span-2">
        <label class="block text-gray-300 text-sm font-bold mb-2">Título de la Promoción</label>
        <input 
          type="text" 
          bind:value={title} 
          required
      ...
      <div>
        <label class="block text-gray-300 text-sm font-bold mb-2">Categoría</label>
        <select 
          bind:value={category}
      ...
      <div>
        <label class="block text-gray-300 text-sm font-bold mb-2">Beneficio</label>
        <input 
          type="text" 
          bind:value={benefit} 
          required
      ...
      <div>
        <label class="block text-gray-300 text-sm font-bold mb-2">Fecha Inicio</label>
        <input 
          type="date" 
      ...
      <div>
        <label class="block text-gray-300 text-sm font-bold mb-2">Fecha Fin</label>
        <input 
          type="date" 
          bind:value={endDate} 
          required
      ...
      <div class="col-span-1 md:col-span-2">
        <label class="block text-gray-300 text-sm font-bold mb-2">URL de Imagen</label>
        <input 
          type="url" 
          bind:value={imageUrl} 
          class="w-full bg-surface-dark text-white border border-gray-600 rounded-lg px-4 py-2 focus:border-primary focus:outline-none"
